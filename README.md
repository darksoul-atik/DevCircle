...
- **RBAC System**: A comprehensive Role-Based Access Control (RBAC) system with separate permissions for **Central Admin** and **Moderators** has not been implemented yet. Currently, the application does not provide granular role-based permissions for administrative and moderation activities.
- **User Blocking**: A user blocking mechanism has not been implemented. Users currently cannot block other users or restrict their interactions with specific accounts.
- **Post Hiding & Moderation**: A post hiding and moderation mechanism has not been implemented. Admins and moderators currently do not have dedicated functionality to hide, review, or manage posts that may violate platform rules.
